...
#define FRAME_HALF (TILE_ID % 2 == 0 ? 0 : 1)

void barrier(uint16_t state)
{
...
int main(void)
{
  uint8_t volatile *mem1 = (uint8_t *)vep_memories_shared_remote_start[(TILE_ID < 2) ? 0 : 2][1];
  uint64_t t;
  t = read_global_timer();
  ...
  while (1)
  {
    ...
    volatile uint8_t *const frameA = (WORK_NO == 0) ? MEM0->pixel_space : (&MEM0->pixel_space[ALLOCATED_BYTES_MEMS] - xsize * ysize);
