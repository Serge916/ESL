...
int main(int argc, char ** argv)
{
  ...
  for (uint32_t f=1; f <= nr_files; f++) {
    ...
    if (bytes_snd > ALLOCATED_BYTES_MEMS){
    	fprintf(stderr, "File exceeds size in memory.\n");
	return 1;
    }
    data_in_shared_mem->current_work.x_size = xsize_snd;
    data_in_shared_mem->current_work.y_size = ysize_snd;
    data_in_shared_mem->current_work.bits_per_pixel = bitsperpixel_snd;
    ...
    while(data_in_shared_mem->current_work.busy == 1){}
    ...
    if (writeBMP(outfile, data_in_shared_mem->pixel_space, xsize_snd, ysize_snd, 8)) {
