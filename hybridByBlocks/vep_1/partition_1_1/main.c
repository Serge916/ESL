...
int main(void)
{
  ...
  while (1)
  {
    ...
    convolution(&frameA[convolution_buffer], x_size, y_size, lines_per_frame, filter, filter_size, &frameB[x_size], FRAME_HALF, initial_y);
    ...
    if (FRAME_HALF == 1)
    {
      ...
      {
        frameA[convolution_buffer - (index + 1)] = frameA[x_size * lines_per_frame - (index + 1)];
      }
      // Sobel
